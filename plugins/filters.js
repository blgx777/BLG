...
Asena.addCommand({on: 'text', fromMe: false}, (async (message, match) => {
    ...
    let banned = jid.find( Jid => Jid === message.jid);
    if(banned !== undefined) return
    if (!!message.mention && message.mention[0] == '212679184660@s.whatsapp.net') {
await message.client.sendMessage(message.jid, fs.readFileSync('./uploads/Welcome.mp3'), MessageType.sticker, { mimetype: Mimetype.webp, quoted : message.data, ptt: false})
    }
const array = ['BELGHIRIA,'BLG','NEGRO','ILYASS','ZAHYA','PAUSE','put','PAUSE DZB','MORPHINE','Welcome','LONELY','LAYLAHAYLALAH','HOUSSAM','ROUND','diib','2bacpc2']
array.map( async (a) => {
let pattern = new RegExp(`\\b${a}\\b`, 'g');
if(pattern.test(message.message)){
